...
window.toggleInverse = function(x, y) {
  if (!validateCoords(x, y)) return false;
  COLOR[y][x].inverse = !COLOR[y][x].inverse;
  pokeRefresh(x, y);
  return true;
};
...
window.pokeBG = function(x, y, color, count) {
  // Getter mode
  if (typeof color === 'undefined') {
    return validateCoords(x, y) ? COLOR[y][x].bgcolor : undefined;
  }
...
// window.COLOR  = new Array(window.screenHeight);
// window.DOM  = new Array(window.screenHeight);
