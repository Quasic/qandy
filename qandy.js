...
function getCursorAttr(val) {
  // Read current global CURSOR if no val provided
  var code = (typeof val === 'number') ? val : (typeof CURSOR === 'number' ? CURSOR : 0);

  // Initialise globals if not already defined (defensive)
  if (typeof cursorInverse === 'undefined') window.cursorInverse = 0;
  if (typeof cursorBold    === 'undefined') window.cursorBold    = 0;
  if (typeof cursorDim     === 'undefined') window.cursorDim     = 0;
  if (typeof cursorItalic  === 'undefined') window.cursorItalic  = 0;
  if (typeof cursorLine    === 'undefined') window.cursorLine    = 0;
  if (typeof cursorBlink   === 'undefined') window.cursorBlink   = 0;

  // Clear all flags
  window.cursorInverse = 0;
  window.cursorBold    = 0;
  window.cursorDim     = 0;
  window.cursorItalic  = 0;
  window.cursorLine    = 0;
  window.cursorBlink   = 0;

  // Helper constants for fallback bitmask mapping (underline=1, blink=2, inverse=4)
  const BM_UNDERLINE = 1;
  const BM_BLINK     = 2;
  const BM_INVERSE   = 4;

  // ATTR constants (if your ATTR uses other masks like 0x0002 for bold, etc.)
  ...
  const ATTR_UNDER    = (window.ATTR_UNDERLINE !== undefined) ? window.ATTR_UNDERLINE : 0x0010;
  ...
  // If it's one of the known legacy codes, map directly
  if (code === 0) {
    // all zero - cursor off
  } else if (code === 1) {
    window.cursorLine = 1;
    ...
    window.cursorLine  = 1;
    window.cursorBlink = 1;
  } else if (code === 4) {
    window.cursorInverse = 1;
  } else if (code === 5) {
    window.cursorInverse = 1;
    window.cursorBlink   = 1;
  } else {
    // Treat code as a bitmask. Support both the simple 1/2/4 bitmask
    // and the ATTR_* bitfield (so bold/dim/italic can be set if present).
    if (code & BM_UNDERLINE) window.cursorLine = 1;
    if (code & BM_BLINK)     window.cursorBlink = 1;
    if (code & BM_INVERSE)   window.cursorInverse = 1;

    // Also check ATTR_* style bits (useful if `code` is an ATTR mask)
    if (code & ATTR_INVERSE) window.cursorInverse = 1;
    if (code & ATTR_BOLD)    window.cursorBold    = 1;
    if (code & ATTR_DIM)     window.cursorDim     = 1;
    if (code & ATTR_ITALIC)  window.cursorItalic  = 1;
    if (code & ATTR_UNDER)   window.cursorLine    = 1;
    if (code & ATTR_BLINK)   window.cursorBlink   = 1;
  }

  // Return the computed flags for convenience
  return {
    cursorInverse: window.cursorInverse,
    cursorBold:    window.cursorBold,
    cursorDim:     window.cursorDim,
    cursorItalic:  window.cursorItalic,
    cursorLine:    window.cursorLine,
    cursorBlink:   window.cursorBlink
  };
}
